...
class MainMenu(QWidget):
    def __init__(self, main):
        ...
        self.application_width = main.width()
    ...
    def configure_elements(self, ratio):
        ...
        self.start_button.move((self.application_width - self.start_button.width()) // 2, int(120 * ratio))
        self.progress_button.setStyleSheet(styles.main_menu_button_style.format(str(int(28 * ratio))))
        self.progress_button.adjustSize()
        self.progress_button.move((self.application_width - self.progress_button.width()) // 2, int(220 * ratio))
        self.settings_button.setStyleSheet(styles.main_menu_button_style.format(str(int(28 * ratio))))
        self.settings_button.adjustSize()
        self.settings_button.move((self.application_width - self.settings_button.width()) // 2, int(320 * ratio))
        self.exit_button.setStyleSheet(styles.main_menu_button_style.format(str(int(28 * ratio))))
        self.exit_button.adjustSize()
        self.exit_button.move((self.application_width - self.exit_button.width()) // 2, int(420 * ratio))
